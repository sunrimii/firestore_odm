...
void main() {
  group('🐛 Multiple Schema Bug Reproduction Tests', () {
    ...
    setUp(() {
      ...
      secondaryODM = FirestoreODM(secondarySchema, firestore: secondaryFirestore);
    });

    group('🔥 Schema Conflict Issues', () {
      test('should reproduce filter selector conflicts between schemas', () async {
        // Create test data
        final user = User(
          id: 'conflict_user',
          name: 'Conflict User',
        ...
        try {
          ...
          final mainPostResults = await mainODM.posts
              .where(($) => $.tags(arrayContains: 'conflict')
      ...
      test('should reproduce converter conflicts between schemas', () async {
        ...
        try {
          ...
          await secondaryODM.secondaryUsers(user.id).update(user);
          final secondaryRetrieved = await secondaryODM.secondaryUsers(user.id).get();
      ...
      test('should reproduce subcollection conflicts between schemas', () async {
        final user = User(
        ...
        try {
          ...
          await secondaryODM.secondaryUsers(user.id).userPosts(userPost.id).update(userPost);
          ...
          expect(secondarySubPost!.title, equals('Subcollection Conflict Post'));
        ...
        try {
          ...
          await Future.wait([
            ...
            secondaryODM.runTransaction((tx) async {
              ...
              await tx.secondaryUsers(user.id).modify((user) => user.copyWith(
          ...
          final secondaryResult = await secondaryODM.secondaryUsers(user.id).get();
    ...
    group('🔍 Code Generation Analysis', () {
      test('should verify no class name conflicts in generated code', () {
        ...
        try {
          ...
          final secondaryUsersType = secondaryODM.secondaryUsers.runtimeType.toString();
