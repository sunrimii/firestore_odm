...
//   ///
//   /// ⚠️  IMPORTANT: This method requires orderBy() to be called first
//   ///
//   /// Example:
//   /// ```dart
//   /// final query = odm.users
//   ///   .orderBy(($) => ($.followers(true), $.name()))  // Required first!
//   ///   .startAt((1000, "Alice"));                      // Now safe to use
//   /// ```
//   @override
//   FirestoreQuery<S, T, O> startAt(O cursorValues) {
//     _validateOrderByRequired('startAt');
//     final valuesList = _extractValuesFromTuple(cursorValues);
//     final newQuery = query.startAt(valuesList);
//     return FirestoreQuery<S, T, O>(collection, newQuery, _orderByConfig, _orderByBuilder);
//   }
...
//   /// final query = odm.users
//   ///   .orderBy(($) => ($.followers(true), $.name()))  // Required first!
//   ///   .startAfter((1000, "Alice"));                   // Now safe to use
//   /// ```
//   @override
//   FirestoreQuery<S, T, O> startAfter(O cursorValues) {
//     _validateOrderByRequired('startAfter');
//     final valuesList = _extractValuesFromTuple(cursorValues);
...
//   /// final query = odm.users
//   ///   .orderBy(($) => ($.followers(true), $.name()))  // Required first!
//   ///   .endAt((500, "Zoe"));                           // Now safe to use
//   /// ```
//   @override
//   FirestoreQuery<S, T, O> endAt(O cursorValues) {
//     _validateOrderByRequired('endAt');
//     final valuesList = _extractValuesFromTuple(cursorValues);
...
//   /// final query = odm.users
//   ///   .orderBy(($) => ($.followers(true), $.name()))  // Required first!
//   ///   .endBefore((500, "Zoe"));                       // Now safe to use
//   /// ```
//   @override
//   FirestoreQuery<S, T, O> endBefore(O cursorValues) {
//     _validateOrderByRequired('endBefore');
//     final valuesList = _extractValuesFromTuple(cursorValues);
...
//   /// ⚠️  IMPORTANT: This method requires orderBy() to be called first
//   ///
//   /// 🧠 SMART BUILDER: Uses the same field extraction logic as orderBy for perfect consistency
//   ///
//   /// Example:
//   /// ```dart
//   /// final query = odm.users
//   ///   .orderBy(($) => ($.followers(true), $.name()))  // Required first!
//   ///   .startAtObject(lastUser);                       // Auto-extracts (lastUser.followers, lastUser.name)
//   /// ```
//   @override
//   FirestoreQuery<S, T, O> startAtObject(T object) {
//     _validateOrderByRequired('startAtObject');
...
//   /// final query = odm.users
//   ///   .orderBy(($) => ($.followers(true), $.name()))  // Required first!
//   ///   .startAfterObject(lastUser);                    // Auto-extracts (lastUser.followers, lastUser.name)
//   /// ```
//   @override
//   FirestoreQuery<S, T, O> startAfterObject(T object) {
//     _validateOrderByRequired('startAfterObject');
//     final values = _extractValuesWithBuilder(object);
...
//   /// final query = odm.users
//   ///   .orderBy(($) => ($.followers(true), $.name()))  // Required first!
//   ///   .endAtObject(firstUser);                        // Auto-extracts (firstUser.followers, firstUser.name)
//   /// ```
//   @override
//   FirestoreQuery<S, T, O> endAtObject(T object) {
//     _validateOrderByRequired('endAtObject');
//     final values = _extractValuesWithBuilder(object);
...
//   /// final query = odm.users
//   ///   .orderBy(($) => ($.followers(true), $.name()))  // Required first!
//   ///   .endBeforeObject(firstUser);                    // Auto-extracts (firstUser.followers, firstUser.name)
//   /// ```
//   @override
//   FirestoreQuery<S, T, O> endBeforeObject(T object) {
//     _validateOrderByRequired('endBeforeObject');
//     final values = _extractValuesWithBuilder(object);
