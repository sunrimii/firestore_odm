...
class ConverterGenerator {
  ...
  static String generateConverterClass(ModelAnalysis analysis) {
    ...
    buffer.writeln('class $converterClassName implements FirestoreConverter<$className, Map<String, dynamic>> {');
  ...
  static String _generateFieldFromFirestoreConversion(
    ...
    // Handle null values
    if (isNullable) {
      final nonNullConversion = _generateNonNullFromFirestoreConversion(field, sourceExpression);
  ...
  static String _generateFieldToFirestoreConversion(
    ...
    // Handle null values
    if (isNullable) {
      final nonNullConversion = _generateNonNullToFirestoreConversion(field, sourceExpression);
      return '$sourceExpression == null ? null : $nonNullConversion';
    }
  ...
  static String _generateListFromFirestoreConversion(
    ...
    return '($sourceExpression as List).cast<${_getListElementType(field)}>()';
  }
  ...
  static String _generateMapFromFirestoreConversion(
    ...
    return '($sourceExpression as Map).cast<String, ${_getMapValueType(field)}>()';
  }
  ...
  static String generateConverterConstants(Map<String, ModelAnalysis> allAnalyses) {
    final buffer = StringBuffer();
    ...
    for (final analysis in allAnalyses.values) {
      ...
      final constantName = '${className[0].toLowerCase()}${className.substring(1)}Converter';
