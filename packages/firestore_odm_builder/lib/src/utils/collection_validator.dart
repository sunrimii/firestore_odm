...
class CollectionValidationInfo {
  ...
  CollectionValidationInfo(this.path, this.isSubcollection, this.element, this.annotationIndex);
}
...
class CollectionValidator {
  static List<CollectionValidationError> validateCollections(LibraryReader library) {
  ...
  static List<CollectionValidationInfo> _extractCollectionInfo(ClassElement element) {
    ...
    for (final annotation in element.metadata) {
      ...
      if (annotationValue != null && collectionChecker.isExactlyType(annotationValue.type!)) {
        ...
        collections.add(CollectionValidationInfo(path, isSubcollection, element, annotationIndex));
        annotationIndex++;
      }
    }
  ...
  static List<CollectionValidationError> _validateClassConfiguration(
    ...
    if (constructor == null) {
      errors.add(CollectionValidationError(
        title: 'Missing Unnamed Constructor',
        description: 'Class ${element.name} with @Collection annotation must have an unnamed constructor.',
        solution: 'Add an unnamed constructor: const factory ${element.name}(...) = _${element.name};',
        element: element,
      ));
    ...
    final hasDocumentIdField = constructor.parameters.any((param) {
      return param.metadata.any((meta) {
        ...
        return metaValue != null && 
               TypeChecker.fromRuntime(DocumentIdField).isExactlyType(metaValue.type!);
      });
    });
    ...
    if (!hasDocumentIdField) {
      errors.add(CollectionValidationError(
        ...
        description: 'Class ${element.name} must have exactly one field marked with @DocumentIdField() annotation.',
        solution: 'Add @DocumentIdField() to one String field: @DocumentIdField() required String id,',
        element: element,
      ));
    }
    ...
    for (final entry in pathCounts.entries) {
      if (entry.value > 1) {
        errors.add(CollectionValidationError(
          ...
          description: 'Class ${element.name} has ${entry.value} @Collection annotations with the same path "${entry.key}".',
          solution: 'Remove duplicate @Collection annotations or use different collection paths.',
          element: element,
  ...
  static List<CollectionValidationError> _validateCollectionPath(CollectionValidationInfo collection) {
    final errors = <CollectionValidationError>[];
    ...
    if (path.trim().isEmpty) {
      errors.add(CollectionValidationError(
        ...
        solution: 'Provide a valid collection path: @Collection("users") or @Collection("users/*/posts")',
        element: element,
        collectionPath: path,
      ));
      return errors;
    ...
    if (path.startsWith('/') || path.endsWith('/')) {
      errors.add(CollectionValidationError(
        ...
        solution: 'Remove leading/trailing slashes: @Collection("users") not @Collection("/users/")',
        element: element,
        collectionPath: path,
      ));
    }
    ...
    if (path.contains('//')) {
      errors.add(CollectionValidationError(
        title: 'Invalid Path Format',
        description: 'Collection path "$path" cannot contain consecutive slashes "//".',
        solution: 'Use single slashes to separate path segments: @Collection("users/posts")',
        element: element,
        collectionPath: path,
      ));
    }
    ...
    for (int i = 0; i < segments.length; i++) {
      if (segments[i].trim().isEmpty && segments[i] != '*') {
        errors.add(CollectionValidationError(
          ...
          description: 'Collection path "$path" has an empty segment at position ${i + 1}.',
          solution: 'Ensure all path segments are non-empty: @Collection("users/posts")',
          element: element,
          collectionPath: path,
        ));
      }
    ...
    if (segments.length % 2 == 0) {
      errors.add(CollectionValidationError(
        ...
        description: 'Collection path "$path" must end with a collection name, not a document ID.',
        solution: 'Collection paths must have odd number of segments. Use "users" not "users/user1", or "users/*/posts" not "users/*/posts/post1"',
        element: element,
        collectionPath: path,
      ));
    }
  ...
  static List<CollectionValidationError> _validateWildcardUsage(CollectionValidationInfo collection) {
    final errors = <CollectionValidationError>[];
    final path = collection.path;
    final element = collection.element;
    final segments = path.split('/');
      ...
      if (segments[i] == '*') {
        if (i % 2 == 0) {
          errors.add(CollectionValidationError(
            ...
            description: 'Wildcard "*" at position ${i + 1} in path "$path" is in a collection position.',
            solution: 'Wildcards must be in document positions (even positions): "users/*/posts" not "*/users/posts"',
            element: element,
            collectionPath: path,
          ));
        }
    ...
    if (segments.isNotEmpty && segments[0] == '*') {
      errors.add(CollectionValidationError(
        ...
        description: 'Collection path "$path" cannot start with a wildcard "*".',
        solution: 'Paths must start with a collection name: "users/*/posts" not "*/posts"',
        element: element,
        collectionPath: path,
      ));
    }
    ...
    if (segments.length < 3 && path.contains('*')) {
      errors.add(CollectionValidationError(
        ...
        description: 'Subcollection path "$path" must have at least 3 segments: collection/*/subcollection.',
        solution: 'Use format like "users/*/posts" with parent collection, wildcard document, and subcollection',
        element: element,
        collectionPath: path,
      ));
    }
  ...
  static List<CollectionValidationError> _validateGlobalConstraints(
    ...
    for (final collection in allCollections) {
      ...
      pathToClasses.putIfAbsent(collection.path, () => <String>{}).add(className);
    }
    ...
    for (final entry in pathToClasses.entries) {
      if (entry.value.length > 1) {
        errors.add(CollectionValidationError(
          ...
          description: 'Collection path "${entry.key}" is used by multiple classes: ${entry.value.join(", ")}.',
          solution: 'Each collection path should only be used by one model class. Use different paths or combine into a single model.',
          collectionPath: entry.key,
        ));
      }
    }
      ...
      if (pathSegments.isNotEmpty) {
        ...
        if (!rootCollections.contains(parentCollection)) {
          errors.add(CollectionValidationError(
            ...
            description: 'Subcollection "${subcollection.path}" references parent collection "$parentCollection" but no @Collection("$parentCollection") exists.',
            solution: 'Create a model with @Collection("$parentCollection") or use an existing parent collection name.',
  ...
  static String generateErrorReport(List<CollectionValidationError> errors) {
    ...
    final buffer = StringBuffer();
    buffer.writeln('🚨 @Collection Configuration Errors Found (${errors.length} issues)');
    buffer.writeln('=' * 80);
    buffer.writeln();
    ...
    for (final entry in errorGroups.entries) {
      buffer.writeln('📋 ${entry.key} (${entry.value.length} occurrence${entry.value.length == 1 ? '' : 's'})');
      buffer.writeln('-' * 40);
    ...
    buffer.writeln('• Add @DocumentIdField() to one String field in your model');
    buffer.writeln('• Use valid collection paths: "users", "users/*/posts"');
    buffer.writeln('• Ensure wildcards (*) are in document positions (even indices)');
    buffer.writeln('• Avoid duplicate collection paths across different classes');
    buffer.writeln('• Create parent collections before referencing in subcollections');
    buffer.writeln();
    buffer.writeln('📖 Documentation: https://github.com/your-repo/firestore-odm#collection-annotation');
