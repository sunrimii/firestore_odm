  ...
  group('Strongly-Typed Pagination System Tests', () {
    ...
    test('single field orderBy with strongly-typed int pagination', () {
      ...
      final query = odm.users
          .orderBy(($) => ($.age(),))  // NEW: Tuple syntax FirestoreQuery<TestSchema, User, (int,)>
    ...
    test('multiple field orderBy with strongly-typed tuple pagination', () {
      ...
      final query = odm.users
          .orderBy(($) => ($.age(), $.name()))  // NEW: Tuple syntax FirestoreQuery<TestSchema, User, (int, String)>
    ...
    test('complex multi-field orderBy with strongly-typed triple tuple pagination', () {
      // Order by rating (double) + age (int) + name (String)
      final query = odm.users
          .orderBy(($) => ($.rating(true), $.age(), $.name()))  // NEW: Tuple syntax (double, int, String)
    ...
    test('nested field orderBy with strongly-typed int pagination', () {
      ...
      final query = odm.users
          .orderBy(($) => ($.profile.followers(true),))  // NEW: Tuple syntax (int,) - nested field!
          .startAt((1000,))     // ✓ Should accept (int,) tuple cursor for followers field
    ...
    test('object-based pagination with automatic cursor extraction', () {
      ...
      final multiQuery = odm.users
          .orderBy(($) => ($.age(), $.name()))  // NEW: Tuple syntax (int, String)
          .startAfterObject(sampleUser);        // Should extract (30, 'John Doe') automatically
      ...
      final tripleQuery = odm.users
          .orderBy(($) => ($.rating(true), $.age(), $.name()))  // NEW: Tuple syntax (double, int, String)
          .endBeforeObject(sampleUser);         // Should extract (4.3, 30, 'John Doe') automatically
    ...
    test('real-world leaderboard pagination with strongly-typed cursors', () {
      ...
      final leaderboard = odm.users
          ...
          .orderBy(($) => ($.rating(true), $.id()))  // NEW: Tuple syntax (double, String)
          .limit(10)
          .startAfter((4.2, 'user_xyz'));  // ✓ (double, String) tuple matches orderBy types
      
      expect(leaderboard, isNotNull);

      // Get next page after getting results
      final nextPage = leaderboard
          .startAfter((3.8, 'user_abc'));  // ✓ Continue with another (double, String) tuple
    ...
    test('chronological pagination with DateTime cursors', () {
      ...
      final recentPosts = odm.users
          ...
          .limit(20)
          .startAfter((DateTime(2023, 12, 1),));  // ✓ (DateTime,) tuple matches orderBy type
      
      expect(recentPosts, isNotNull);

      // Get older posts
      final olderPosts = recentPosts
          .startAfter((DateTime(2023, 6, 1),));  // ✓ Continue with another (DateTime,) tuple
    ...
    test('complex 4-field orderBy with strongly-typed quad tuple pagination', () {
      // Order by 4 fields to test larger tuple types
      final query = odm.users
          .orderBy(($) => ($.rating(true), $.age(), $.name(), $.id()))  // NEW: Tuple syntax (double, int, String, String)
          .startAfter((4.5, 30, 'John', 'user123'))  // ✓ (double, int, String, String) tuple
          .endAt((2.0, 60, 'Zoe', 'user999'));       // ✓ (double, int, String, String) tuple
    ...
    test('demonstrates type evolution through orderBy chain', () {
      ...
      final tupleQuery = multiQuery.startAt((25, 'Alice'));  // (int, String) tuple cursor
      ...
      final tripleFieldQuery = tripleQuery.startAt((25, 'Alice', 4.5));  // (int, String, double) tuple
