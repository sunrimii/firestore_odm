...
void main() {
  group('🔧 Incremental Modify Nested Fields Tests', () {
    ...
    group('📂 Simple Nested Field Atomic Operations', () {
      test('should handle numeric increments in nested fields', () async {
        ...
        await odm.users(user.id).incrementalModify((user) => user.copyWith(
          ...
          profile: user.profile.copyWith(
            followers: user.profile.followers + 250, // ✅ Nested numeric increment
          ),
        ));
      ...
      test('should handle array operations in nested fields', () async {
        ...
        await odm.users(user.id).incrementalModify((user) => user.copyWith(
          ...
          profile: user.profile.copyWith(
            interests: [...user.profile.interests, 'new-interest'], // ✅ Nested array union
          ),
        ));
        ...
        expect(updatedUser.profile.interests, hasLength(3)); // 'coding', 'testing', 'new-interest'
      ...
      test('should handle map operations in nested fields', () async {
        ...
        final updatedUser = await odm.users(user.id).get();
        expect(updatedUser, isNotNull);
        expect(updatedUser!.profile.socialLinks['twitter'], equals('@map')); // Preserved
        expect(updatedUser.profile.socialLinks['github'], equals('map-user')); // Added
        expect(updatedUser.profile.socialLinks['linkedin'], equals('map-professional')); // Added
    ...
    group('🔀 Mixed Nested Field Operations', () {
      test('should handle multiple types of nested operations together', () async {
        final user = User(
        ...
        await odm.users(user.id).incrementalModify((user) => user.copyWith(
          ...
          // ✅ Nested operations
          profile: user.profile.copyWith(
            followers: user.profile.followers + 400, // ✅ Nested numeric increment
            interests: [...user.profile.interests, 'advanced'], // ✅ Nested array union
            socialLinks: {
              ...user.profile.socialLinks,
              'advanced': 'nested-link', // ✅ Nested map update
            },
            bio: '${user.profile.bio} - Enhanced!', // ✅ Nested string update
          ),
        ));
        ...
        expect(updatedUser.profile.socialLinks['advanced'], equals('nested-link'));
      ...
      test('should handle array removal in nested fields', () async {
        final user = User(
          ...
          profile: Profile(
            ...
            interests: ['keep-interest', 'remove-interest', 'also-keep'], // ✅ Nested array for removal
            followers: 800,
            lastActive: DateTime.now(),
          ),
          rating: 4.2,
        ...
        await odm.users(user.id).incrementalModify((user) => user.copyWith(
          ...
          profile: user.profile.copyWith(
            ...
            socialLinks: Map.fromEntries(
              user.profile.socialLinks.entries
                  .where((entry) => entry.key != 'remove'),
            ),
        ...
        expect(updatedUser.profile.interests, isNot(contains('remove-interest')));
    ...
    group('⚡ Bulk Nested Operations', () {
      test('should handle bulk nested incrementalModify on queries', () async {
        ...
        await odm.users
            ...
            .incrementalModify((user) => user.copyWith(
              ...
              profile: user.profile.copyWith(
                followers: user.profile.followers + 500, // ✅ Bulk nested increment
                interests: [...user.profile.interests, 'premium'], // ✅ Bulk nested array union
                socialLinks: {
                  ...user.profile.socialLinks,
                  'premium': 'upgraded', // ✅ Bulk nested map update
                },
                bio: '${user.profile.bio} - Premium!', // ✅ Bulk nested string update
              ),
            ));
        ...
        for (int i = 0; i < updatedUsers.length; i++) {
          ...
          // Nested bulk results
          expect(user.profile.followers, equals(600 + (100 * i))); // 100*(i+1)+500 = 600+100*i
      ...
      test('should handle bulk nested operations on OrderedQuery', () async {
        ...
        await odm.users
            ...
            .incrementalModify((user) => user.copyWith(
              ...
              profile: user.profile.copyWith(
                ...
                followers: user.profile.followers + (user.profile.followers ~/ 10),
        ...
        final highUser = updatedUsers.firstWhere((u) => u.name == 'High Follower');
        final mediumUser = updatedUsers.firstWhere((u) => u.name == 'Medium Follower');
        final lowUser = updatedUsers.firstWhere((u) => u.name == 'Low Follower');
      ...
      test('should handle nested operations with null safety', () async {
        ...
        await odm.users(user.id).incrementalModify((user) => user.copyWith(
          profile: user.profile.copyWith(
            ...
            interests: [...user.profile.interests, 'first'], // ✅ Add to empty array
            socialLinks: {
              ...user.profile.socialLinks,
        ...
        expect(updatedUser.profile.interests, equals(['first'])); // Empty + 'first'
        expect(updatedUser.profile.socialLinks['first'], equals('link')); // Empty + entry
      ...
      test('should handle server timestamps in nested operations', () async {
        ...
        await odm.users(user.id).incrementalModify((user) => user.copyWith(
          ...
          profile: user.profile.copyWith(
            ...
            lastActive: FirestoreODM.serverTimestamp, // ✅ Nested server timestamp
            interests: [...user.profile.interests, 'server-updated'], // ✅ Nested array update
          ),
        ));
