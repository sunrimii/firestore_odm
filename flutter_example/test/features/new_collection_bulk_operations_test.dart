  ...
  group('🏗️ New Collection Bulk Operations', () {
    ...
    group('🔄 Collection Bulk Modify', () {
      test('should perform bulk modify on entire collection', () async {
        ...
        print('✅ FirestoreCollection.modify() - Bulk modify entire collection works');
      });
      ...
      test('should handle empty collection in bulk modify', () async {
        ...
        print('✅ FirestoreCollection.modify() on empty collection handled gracefully');
      });
    });

    group('⚡ Collection Bulk Incremental Modify', () {
      test('should perform bulk incrementalModify with atomic operations', () async {
        // Create test users
        final users = [
          User(
            id: 'inc_col_1',
        ...
        await odm.users.incrementalModify((user) => user.copyWith(
          ...
          rating: user.rating + 0.5, // Should auto-convert to FieldValue.increment(0.5)
          profile: user.profile.copyWith(
            followers: user.profile.followers + 50, // Should auto-convert to FieldValue.increment(50)
          ),
        ));
        ...
        for (final user in incrementedUsers) {
          ...
          expect(user.rating, greaterThanOrEqualTo(3.5)); // 3.0+0.5, 4.0+0.5, 4.5+0.5
          // Followers should be incremented by 50
          expect(user.profile.followers, greaterThanOrEqualTo(150)); // 100+50, 150+50, 200+50
        }

        print('✅ FirestoreCollection.incrementalModify() - Atomic operations work on entire collection');
      });

      test('should handle complex atomic operations in incrementalModify', () async {
        ...
        await odm.users.incrementalModify((user) => user.copyWith(
          ...
          profile: user.profile.copyWith(
            followers: user.profile.followers + 100, // Nested numeric increment
          ),
        ));
        ...
        for (final user in updatedUsers) {
          ...
          expect(user.profile.followers, greaterThanOrEqualTo(600)); // 500+100 or 750+100
        }

        print('✅ FirestoreCollection.incrementalModify() - Complex atomic operations work');
      });
      ...
      test('should handle server timestamps in incrementalModify', () async {
        ...
        print('✅ FirestoreCollection.incrementalModify() - Server timestamps work');
      });
      ...
      test('should handle empty collection in incrementalModify', () async {
        ...
        print('✅ FirestoreCollection.incrementalModify() on empty collection handled gracefully');
      });
    });
    ...
    group('🔀 Mixed Collection Operations', () {
      ...
      test('should work with aggregations after bulk operations', () async {
        ...
        final aggregateResult = await odm.users.aggregate(($) => (
          count: $.count(),
          avgAge: $.age.average(), // Should be (25+26+27+28+29)/5 = 27
          avgRating: $.rating.average(), // Should be (4.0+4.5+5.0+5.5+6.0)/5 = 5.0
          totalFollowers: $.profile.followers.sum(), // Should be 100+200+300+400+500 = 1500
        )).get();

        expect(aggregateResult.count, equals(5));
        expect(aggregateResult.avgAge, equals(27.0)); // (25+26+27+28+29)/5
        expect(aggregateResult.avgRating, equals(5.0)); // (4.0+4.5+5.0+5.5+6.0)/5
        expect(aggregateResult.totalFollowers, equals(1500)); // 100+200+300+400+500
