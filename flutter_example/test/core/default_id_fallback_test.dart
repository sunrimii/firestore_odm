...
void main() {
  group('🔄 Default ID Fallback Tests', () {
    ...
    group('📋 Models Without @DocumentIdField Annotation', () {
      test('SimpleStory model: ODM operations with automatic ID field detection', () async {
        ...
        final updatedStory = story.copyWith(title: 'Updated Default ID Story');
      ...
      test('SimpleStory vs User: Compare ODM behavior with/without @DocumentIdField', () async {
        ...
        final storyQuery = await odm.simpleStories
            .where((filter) => filter.authorId(isEqualTo: 'comparison_author_123'))
            .get();
      ...
      test('SimpleStory ordering and pagination without @DocumentIdField', () async {
    ...
    group('🔍 Mixed Annotation Scenarios', () {
      test('Mixing annotated and non-annotated models in same ODM', () async {
        ...
        final orderedStories = await odm.simpleStories.orderBy(($) => ($.id(),)).get();
      ...
      test('Subcollections with mixed annotation models', () async {
        ...
        final userPostsById = await odm.users(userId).posts
            .where((filter) => filter.id(isEqualTo: postId))
            .get();
      ...
      test('Verify document ID field detection priority works correctly', () async {
